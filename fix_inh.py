...
Retourne la premiere liste de valeurs trouvee (non-nil)."
...
Si if-needed donne une valeur, retourne cette valeur (et ne la met pas automatiquement)."
  (or (Fget-I frame slot 'value)
      ...
      (let ((res (Fget-I frame slot 'if-needed)))
        (when res
          ;; appeler les demons et renvoyer leur sortie (premiere non-nil)
          (let ((out nil))
...
(defun Fget-Z (frame slot)
  "Recherche largeur d'abord; a chaque niveau: value, defaut, if-needed.
Retourne la premiere liste/valeur trouvee."
  (let ((seen (list frame))
        (level (list frame)))
    (loop while level
          do (progn
